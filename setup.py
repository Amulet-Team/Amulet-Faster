...
class CMakeBuild(BuildExt):
    def build_extension(self, ext: Extension) -> None:
        ...
        faster_src_dir = (
            Path.cwd() / "src" / "amulet" / "faster" if self.editable_mode else ext_dir
        )
            ...
            if subprocess.run(
                [
                    ...
                    *platform_args,
                    ...
                    f"-Damulet_faster_DIR={fix_path(faster_src_dir)}",
                    f"-DAMULET_FASTER_EXT_DIR={fix_path(ext_dir)}",
                    f"-DCMAKE_INSTALL_PREFIX=install",
                    "-B",
                    tempdir,
                ]
            ).returncode:
                raise RuntimeError("Error configuring amulet-faster")
            if subprocess.run(
                ["cmake", "--build", tempdir, "--config", "Release"]
            ).returncode:
                raise RuntimeError("Error building amulet-faster")
            if subprocess.run(
                ["cmake", "--install", tempdir, "--config", "Release"]
            ).returncode:
                raise RuntimeError("Error installing amulet-faster")
...
setup(
    ...
    cmdclass=cmdclass,
    ext_modules=[Extension("amulet.faster._faster", [])]
